    ...
    return (
        <div className="flex h-screen">
            <div className={`bg-white flex flex-col ${isMobile? (selectedUserId ? "hidden" : "w-full") : "w-1/3" } md:flex`}>
                {/*LOGO*/}
                <div className={`sticky top-0 text-[#ED7A46] font-bold text-lg flex gap-2 p-4 pb-10 ${isMobile? "mt-10" : ""}`}>
                ...
                {/* my profile section */}
                <div className={`fixed sticky bottom-0 bg-gray-100 h-10 rounded-full m-5 ${ isMobile? "mb-10": "mb-2"} p-2 text-center flex items-center justify-center`}>
            ...
            <div className={`bg-[#FDF3E5] flex flex-col ${ isMobile ? (selectedUserId ? "w-full pt-10" : "hidden") : "w-2/3"}`}>
                {!!selectedUserId && (
                    // Chatting user info
                ...
                {!!selectedUserId && (
                    <form className={`flex gap-2 p-2 ${isMobile? "sticky bottom-0 mb-10" : ""}`} onSubmit={sendMessage}>
                        <input type="text"
